...
- 使用 ddd 规范组织代码目录结构
- 用 `mv` 到 bak 目录 代替删除
- 修改代码(包括测试代码)后, 先执行 `pnpm check` (会依次执行类型检查和代码风格检查)；验证通过才输出**单独一行**的简要的一句话 git commit 信息，不需要引号，方便直接复制
...
- 使用 package.json 里的命令，不要使用变体
  - pnpm e2e [*] 会先build后测试；如果直接使用变体命名，可能导致沿用旧代码
