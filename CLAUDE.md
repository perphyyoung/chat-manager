...
- 使用 ddd 规范组织代码目录结构
- 用 `mv` 到 bak 目录 代替删除
- 修改代码(包括测试代码)后, 先执行 `pnpm check` (会依次执行类型检查和代码风格检查)，通过后再继续
- 添加必要的注释, 说明为什么, 而非是什么
- 优先修改, 而非重写
...
- 搜索总是优先使用 gitnexus, 其次是 rg, 而不是默认的 grep
- 调用 gitnexus 时，参数添加 `"repo": "chat-manager"` 指定当前仓库
- 如果 gitnexus 索引过期，执行带嵌入的 `gitnexus analyze --embeddings` 重建索引，然后重新搜索
